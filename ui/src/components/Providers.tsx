...
export function Providers() {
  ...
  const [editingProviderIndex, setEditingProviderIndex] = useState<
    number | null
  >(null);
  const [deletingProviderIndex, setDeletingProviderIndex] = useState<
    number | null
  >(null);
  const [hasFetchedModels, setHasFetchedModels] = useState<
    ...
    const fetchProviderTemplates = async () => {
      try {
        const response = await fetch(
          "https://pub-0dc3e1677e894f07bbea11b17a29e032.r2.dev/providers.json"
        );
        if (response.ok) {
  ...
  const validProviders = Array.isArray(config.Providers)
    ? config.Providers
    : [];
  ...
  const handleSaveProvider = () => {
    ...
    if (
      ...
      editingProviderData.api_key.trim() === ""
    ) {
  ...
  const handleProviderTransformerChange = (
    _index: number,
  ...
  const removeProviderTransformerAtIndex = (
    _index: number,
    ...
    if (updatedProvider.transformer) {
      ...
      if (
        newUseArray.length === 0 &&
        Object.keys(updatedProvider.transformer).length === 1
      ) {
  ...
  const handleModelTransformerChange = (
  ...
  const removeModelTransformerAtIndex = (
    _providerIndex: number,
    model: string,
    transformerIndex: number
  ) => {
    ...
    if (updatedProvider.transformer && updatedProvider.transformer[model]) {
      ...
      if (
        newUseArray.length === 0 &&
        Object.keys(updatedProvider.transformer[model]).length === 1
      ) {
  ...
  const addProviderTransformerParameter = (
    ...
    if (
      updatedProvider.transformer.use &&
      updatedProvider.transformer.use.length > transformerIndex
    ) {
      const targetTransformer =
        updatedProvider.transformer.use[transformerIndex];
      ...
      if (Array.isArray(targetTransformer)) {
        ...
        if (
          transformerArray.length > 1 &&
          typeof transformerArray[1] === "object" &&
  ...
  const removeProviderTransformerParameterAtIndex = (
    ...
    if (Array.isArray(targetTransformer) && targetTransformer.length > 1) {
      ...
      if (
        ...
        const paramsObj = {
          ...(transformerArray[1] as Record<string, unknown>),
        };
  ...
  const addModelTransformerParameter = (
    ...
    if (
      ...
      const targetTransformer =
        updatedProvider.transformer[model].use[transformerIndex];
  ...
  const handleAddModel = (_index: number, model: string) => {
    ...
    const models = Array.isArray(updatedProvider.models)
  ...
  const editingProvider =
    ...
    (editingProviderIndex !== null
      ? validProviders[editingProviderIndex]
      : null);
  ...
  const filteredProviders = validProviders.filter((provider) => {
    ...
    if (
      ...
      (provider.api_base_url &&
        provider.api_base_url.toLowerCase().includes(term))
    ) {
      return true;
    }
    // Check models
    if (provider.models && Array.isArray(provider.models)) {
      return provider.models.some(
        (model) => model && model.toLowerCase().includes(term)
      );
    }
    return false;
  });
            ...
            <div className="space-y-4 p-4 overflow-y-auto flex-grow">
              {providerTemplates.length > 0 && (
                <div className="space-y-2">
                  ...
                  <Combobox
                    options={providerTemplates.map((p) => ({
                      label: p.name,
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <div className="flex gap-2">
                    <div className="flex-1">
                      {hasFetchedModels[editingProviderIndex] ? (
                        <ComboInput
                          ref={comboInputRef}
                          options={(editingProvider.models || []).map(
                            (model: string) => ({ label: model, value: model })
                          )}
                          value=""
                          onChange={() => {
                            // 只更新输入值，不添加模型
                    ...
                    <Button
                      onClick={() => {
                        if (
                          ...
                          // 使用ComboInput的逻辑
                          const comboInput =
                            comboInputRef.current as unknown as {
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    {(editingProvider.models || []).map(
                      (model: string, modelIndex: number) => (
                        <Badge
                          key={modelIndex}
                          variant="outline"
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Combobox
                    ...
                    onChange={(value) => {
                      if (editingProviderIndex !== null) {
                        handleProviderTransformerChange(
                          editingProviderIndex,
                          value
                        );
                      }
                ...
                {editingProvider.transformer?.use &&
                  editingProvider.transformer.use.length > 0 && (
                            ...
                            <div className="mt-2 pl-4 border-l-2 border-gray-200">
                              ...
                              <div className="space-y-2 mt-1">
                                ...
                                {(() => {
                                  ...
                                  if (
                                    !editingProvider.transformer?.use ||
                                    editingProvider.transformer.use.length <=
                                      transformerIndex
                                  ) {
                                    return null;
                                  }
                                  ...
                                  if (
                                    ...
                                    // Check if the second element is an object (parameters object)
                                    if (
                                      typeof targetTransformer[1] ===
                                        "object" &&
                                      targetTransformer[1] !== null
                                    ) {
                                      params = targetTransformer[1] as Record<
                                        string,
                                        unknown
                                      >;
                                    }
                      ...
                      (model: string, modelIndex: number) => (
                        <div key={modelIndex} className="border rounded-md p-3">
                          ...
                          {editingProvider.transformer?.[model]?.use &&
                            editingProvider.transformer[model].use.length >
                              0 && (
                              <div className="space-y-2 mt-2">
                                <div className="text-sm font-medium text-gray-700">
                                  {t("providers.selected_transformers")}
                                  ...
                                  (
                                    ...
                                    >
                                      ...
                                      <div className="mt-2 pl-4 border-l-2 border-gray-200">
                                        <Label className="text-sm">
                                          {t(
                                            "providers.transformer_parameters"
                                          )}
                                        </Label>
                                        <div className="space-y-2 mt-1">
                                          <div className="flex gap-2">
                                          ...
                                          {(() => {
                                            // Get parameters for this specific transformer
                                            if (
                                              !editingProvider.transformer?.[
                                                model
                                              ]?.use ||
                                              editingProvider.transformer[model]
                                                .use.length <= transformerIndex
                                            ) {
                                              return null;
                                            }
      ...
      >
        <DialogContent>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setDeletingProviderIndex(null)}
            >
              {t("providers.cancel")}
            </Button>
            <Button
